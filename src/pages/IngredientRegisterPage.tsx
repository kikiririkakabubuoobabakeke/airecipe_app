...
カテゴリは次から選んでください: 肉・卵・魚, 野菜, 乳製品, 主食, 調味料, 加工品, 飲料, その他
...
function inferFoodCategory(name: string, category: unknown) {
  ...
  if (value === 'staple') return '主食'
  if (value === 'seasoning') return '調味料'
  if (value === 'processed') return '加工品'
  if (value === 'drink' || value === 'beverage') return '飲料'
  ...
  if (/(米|白米|ご飯|パン|麺|うどん|そば|パスタ|ラーメン|焼きそば)/u.test(name)) {
    return '主食'
  }

  if (/(醤油|しょうゆ|味噌|みそ|塩|砂糖|油|ごま油|酢|ソース|だし|コンソメ|ケチャップ|マヨネーズ|カレー粉|こしょう|胡椒|料理酒|みりん|ポン酢|めんつゆ)/u.test(name)) {
    return '調味料'
  }

  if (/(納豆|豆腐|ちくわ|缶|冷凍|惣菜|加工|レトルト|カップ)/u.test(name)) {
    return '加工品'
  }

  if (/(茶|水|ジュース|飲料|コーヒー|珈琲|ソーダ)/u.test(name)) {
    return '飲料'
  }
